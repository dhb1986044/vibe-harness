...
Task arrives
    ...
    └── Governance (vibe-*)
        ├── Changing AGENTS/memory-bank/LESSONS/evolution? ─→ vibe-memory-check
        ├── About to make a high-risk change? ──────────────→ vibe-guard
        ├── Verifying after EXEC, before COMPLETE? ─────────→ vibe-xcheck
        └── Deciding if a lesson should be promoted? ───────→ vibe-evolve
```
