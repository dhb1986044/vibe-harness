...
def parse_index(text: str):
    ...
    for line in text.splitlines():
        ...
        if in_index and line.startswith("## "):
            break
        if not in_index or not line.startswith("|") or "---" in line or "标题" in line:
            continue
        cols = [c.strip() for c in line.strip().strip("|").split("|")]
        if len(cols) >= 5 and re.fullmatch(r"L\d+", cols[0]):
            rows.append({"id": cols[0], "title": cols[1], "tags": cols[2], "priority": cols[3], "status": cols[4]})
    return rows
...
def main():
    ...
    rows = parse_index(LESSONS.read_text(encoding="utf-8"))
    data = {"schema_version": "v5", "updated_at": str(date.today()), "lessons": []}
    candidates = []
    for row in rows:
        s = score(row)
        t = target(row)
        item = {
            ...
            "promotion_score": s, "promotion_target": t,
            "promotion_status": "candidate" if s >= 10 and t != "lesson" else "keep_as_lesson",
    ...
    if args.write:
        ...
        with PROMOTION_LOG.open("a", encoding="utf-8") as f:
            f.write(f"\n## {date.today()}\n- Generated {len(candidates)} promotion candidates.\n")
